...
def main() -> int:
    ...
    agents = ROOT / ".opencode" / "agents"; commands = ROOT / ".opencode" / "commands"; skills = ROOT / ".opencode" / "skills"
    ...
    for name in ("source", "inputs", "workspace", "outputs", "projects", "assets"):
        path = ROOT / name; checks.append(check(f"space_{name}", "PASS" if path.is_dir() else "FAIL", str(path), "managed"))
    try:
    ...
    for label, command in (("runctl", [sys.executable, str(ROOT / "runtime" / "runctl.py"), "--help"]), ("pangea_cli", [sys.executable, "-m", "tooling.pangea_cli", "project", "--help"])):
    ...
    payload: dict[str, Any] = {"artifact_type": "doctor_report", "schema_version": "1.1", "direct_expert_mode": "AVAILABLE" if not direct_failures else "UNAVAILABLE", "managed_task_mode": "AVAILABLE" if not direct_failures and not managed_failures else "UNAVAILABLE", "checks": checks}
