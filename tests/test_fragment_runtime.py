...
class RuntimeTests(unittest.TestCase):
 def base(self,resource=False):
  ...
  lines=(ROOT/item["path"]).read_text().splitlines() or [""];text="\n".join(lines[item["line_start"]-1:item["line_end"]]);fact={"obligation_id":row["obligation_id"],"inventory_id":item["inventory_id"],"path":item["path"],"line_start":item["line_start"],"line_end":item["line_end"],"excerpt_sha256":hashlib.sha256(text.encode()).hexdigest(),"evidence":"source"};key=[row["obligation_id"],item["inventory_id"],item["line_start"],item["line_end"]]
  frag={"artifact_type":"analysis_fragment","schema_version":"1.0","worker_instance":"worker","run_id":"run","fragment_id":"frag","context_pack_sha256":context_budget.digest(pack),"obligation_ids":[row["obligation_id"]],"skill_receipt_ids":[r["receipt_id"] for r in rs],"facts":[fact],"contributions":{"flows":[{"priority":"P0","obligation_id":row["obligation_id"],"controls":["cli"],"oracles":["exit"],"fact_key":key}]},"risk_cards":[{"severity":"Low","obligation_id":row["obligation_id"],"fact_key":key,"summary":"observed"}],"dispositions":[{"obligation_id":row["obligation_id"],"outcome":"analyzed","reason":"source"}],"unresolved":[],"usage":{"output_tokens":10,"finish_reason":"stop","valid_json":True}}
  return inv,led,pack,rs,fragment_runtime.upgrade_legacy(frag)
 ...
 def test_fragment_covered_cycle_rejected(self):
  ...
  with self.assertRaises(fragment_runtime.FragmentError):fragment_runtime.validate(frag,pack,inv,led,ROOT,[],{})
 def test_covered_by_existing_analyzed_fragment_happy(self):
  inv=source_inventory.build(ROOT,"mini","c"*40);led=obligation_ledger.build(inv,ROOT);item=next(i for i in inv["items"] if i["kind"]=="entrypoint" and not i["storage_skill_triggers"]);rows=[r for r in led["obligations"] if r["inventory_id"]==item["inventory_id"]][:2];rg={k:item[k] for k in ("inventory_id","path","line_start","line_end")};pack=context_budget.build(inv,led,ROOT,[rows[0]["obligation_id"]],[rg],[],{},"run","frag1");line=(ROOT/item["path"]).read_text().splitlines()[item["line_start"]-1];fact={"obligation_id":rows[0]["obligation_id"],"inventory_id":item["inventory_id"],"path":item["path"],"line_start":item["line_start"],"line_end":item["line_end"],"excerpt_sha256":hashlib.sha256(line.encode()).hexdigest(),"evidence":"source"};first={"artifact_type":"analysis_fragment","schema_version":"1.0","worker_instance":"worker1","run_id":"run","fragment_id":"frag1","context_pack_sha256":context_budget.digest(pack),"obligation_ids":[rows[0]["obligation_id"]],"skill_receipt_ids":[],"facts":[fact],"contributions":{"flows":[]},"risk_cards":[],"dispositions":[{"obligation_id":rows[0]["obligation_id"],"outcome":"analyzed","reason":"source"}],"unresolved":[],"usage":{"output_tokens":8,"finish_reason":"stop","valid_json":True}};after=fragment_runtime.validate_and_apply(led,first,pack,inv,ROOT,[],{});target=rows[0]["obligation_id"];second_row=rows[1];pack2=context_budget.build(inv,after,ROOT,[second_row["obligation_id"]],[rg],[],{},"run","frag2")
  second={"artifact_type":"analysis_fragment","schema_version":"1.0","worker_instance":"worker2","run_id":"run","fragment_id":"frag2","context_pack_sha256":context_budget.digest(pack2),"obligation_ids":[second_row["obligation_id"]],"skill_receipt_ids":[],"facts":[],"contributions":{"flows":[]},"risk_cards":[],"dispositions":[{"obligation_id":second_row["obligation_id"],"outcome":"covered_by_other","reason":"same inventory behavior","covered_by":target}],"unresolved":[],"usage":{"output_tokens":8,"finish_reason":"stop","valid_json":True}}
  merged=fragment_runtime.validate_and_apply(after,second,pack2,inv,ROOT,[],{});self.assertEqual("complete",next(r for r in merged["obligations"] if r["obligation_id"]==second_row["obligation_id"])["status"])
