---
description: 对指定模块执行完整或快速的 obligation 驱动测试分析
agent: pangea-test
---
...
2. 运行时集成契约（R2）：先生成独立 inventory/obligation ledger，再按 obligation/range 创建 immutable context pack，由隔离执行器并发派发同一个 `analysis-worker`，同时注入适用 capability pack 与 Storage Skill receipt；fragment、runner telemetry 与 evaluator 签名执行收据必须完整绑定后才可合并。
3. 默认完整型覆盖六个 capability packs；`--fast` 保留同样的 obligation 覆盖但限制调用链展开和分支深度，报告中必须写明边界。
